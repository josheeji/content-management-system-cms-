...
@section('content')
    <div class="card">
        <div class="card-body">
            ...
            <a href="/auth/banners/{{ $banner->id }}/banner-items"> <button type="button" class="btn btn-success">Back
                </button> </a>
            <hr>
            @if (session('message'))
                <h6 class="alert alert-success">{{ session('message') }} </h6>
            @endif
            <form class="form-sample" action="/auth/banners/{{ $banner->id }}/banner-items/{{ $bannerItem->id }}"
